...
from app.models.entities import AuditLog, Backup, Subnet, User, VPNPeer, VPNServer
...
def login(request: Request, username: str = Form(...), password: str = Form(...), db: Session = Depends(get_db)):
    ...
    is_admin_username = username == settings.admin_username

    if not user and is_admin_username:
        bootstrap_password = password or settings.admin_password
        user = User(username=settings.admin_username, password_hash=hash_password(bootstrap_password), role="admin")
        db.add(user)
        db.commit()
        db.refresh(user)
    ...
    # Fallback 1: allow login via ADMIN_PASSWORD from .env and resync hash.
    if user and is_admin_username and not password_ok and password == settings.admin_password:
        user.password_hash = hash_password(settings.admin_password)
        db.commit()
        password_ok = True

    # Fallback 2 (DEMO-only): never block admin demo login; rotate hash to entered value.
    if user and is_admin_username and not password_ok and detect_mode() == "DEMO":
        user.password_hash = hash_password(password or settings.admin_password)
        db.commit()
        password_ok = True
    ...
    if not password_ok or not user:
        return templates.TemplateResponse(
            "login.html",
            _ctx(request, error="Неверный логин или пароль. Проверьте ADMIN_USERNAME/ADMIN_PASSWORD в .env."),
        )

    response = RedirectResponse(url="/", status_code=303)
    response.set_cookie("session", create_session_token(user.username), httponly=True, samesite="lax", path="/")
    return response
...
def dashboard(request: Request, _: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    stats = statistics_service.get_summary(db, hours=12)
    return templates.TemplateResponse(
        "dashboard.html",
        _ctx(request, servers=servers, clients=clients, snaps=snaps, logs=logs, active=active, stats=stats),
    )
...
def monitoring_page(request: Request, _: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    stats = statistics_service.get_summary(db, hours=24)
    return templates.TemplateResponse("monitoring.html", _ctx(request, snaps=snaps, stats=stats))


@router.get("/monitoring/stats", response_class=JSONResponse)
def monitoring_stats(_: User = Depends(get_current_user), db: Session = Depends(get_db)):
    return JSONResponse(statistics_service.get_summary(db, hours=24))
