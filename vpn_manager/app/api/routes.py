...
def login(request: Request, username: str = Form(...), password: str = Form(...), db: Session = Depends(get_db)):
    ...
    # Demo-friendly fallback: if user was not initialized correctly, allow .env admin credentials.
    if not user and username == settings.admin_username and password == settings.admin_password:
    ...
    if not password_ok and username == settings.admin_username and password == settings.admin_password and user:
    ...
    response.set_cookie("session", create_session_token(user.username), httponly=True, samesite="lax")
    return response
